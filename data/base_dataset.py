...
from data.utils.transforms import augment, paired_random_crop, center_crop
...
class BaseDataset(Dataset):
    ...
    def _process_images(self, img_gt, img_lq):
        ...
        if self.mean is not None or self.std is not None:
            img_lq = (img_lq - jt.array(self.mean).reshape(-1, 1, 1)) / jt.array(self.std).reshape(-1, 1, 1)
            img_gt = (img_gt - jt.array(self.mean).reshape(-1, 1, 1)) / jt.array(self.std).reshape(-1, 1, 1)
        return img_gt, img_lq
