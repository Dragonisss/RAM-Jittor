...
from models.mim_util import MaskGenerator, TestMaskGenerator
...
class RAMPretrainModel(RAMBaseModel):
    ...
    def feed_data(self, data):
        ...
        if hasattr(self, "mask_generator"):
            ...
            self.mask = mask.expand(self.gt.shape[0], -1, -1)
            if mask_token is not None:
                self.mask_token = mask_token.expand(self.gt.shape[0], -1, -1, -1)
            else:
    ...
    def test(self):
        with jt.no_grad():
            if hasattr(self, 'net_g_ema'):
                ...
                mask = self.mask.repeat_interleave(1, 1).repeat_interleave(1, 2).unsqueeze(1)     
