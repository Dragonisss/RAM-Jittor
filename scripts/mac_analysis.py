...
class MACAnalysis(BaseAnalysis):
    ...
    def analyze(self):
        ...
        for test_loader in self.test_loaders:
            ...
            for idx, val_data in enumerate(test_loader):
                if idx >= num_samples:
                    break
                tensor_lq = val_data['lq']
                imgname = osp.basename(val_data['lq_path'][0])
                tensor_base = jt.zeros_like(tensor_lq)
    ...
    def _mask_attribute_conductance(self, base_img, final_img):
        ...
        for step in range(total_step):
            ...
            interpolated_img = self._get_interpolated_img_from_mask_attribute_path(base_img, final_img, alpha, order_array)
            interpolated_output = self.model(interpolated_img)
            
            loss = attr_grad(interpolated_output,reduce='sum')
            self.optimizer.backward(loss,retain_graph=False)
            self.model.eval()
        ...
        return [jt.mean(jt.abs(cond)).item() for cond in all_hook_layer_conductance]
